...
5. Define `scenes/<scene_id>/scene.yaml` plus assets for static geometry, expected frames, simulator world settings, and default object placements.
6. Mark every combination as `supported_sim`, `commissioned_real`, `unsupported`, or `unverified`. Simulation support is required for all twelve robot/tool selections.
7. Provide a profile-inspection command or generated report that Phase 01 can consume when implementing validation.
