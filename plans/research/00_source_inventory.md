...
The current manipulation package exposes `Robot`, `Pose`, `Joints`, arm motion, planning and execution, grippers, diagnostics, background-process ownership, network configuration, and task examples. Reuse its proven robotics behavior without copying its architecture wholesale. Characterization fixtures must cover:
...
- launch-process ownership, explicit attach, refusal to take over an unrelated stack, and signal cleanup;
...
The legacy FastAPI process also serves the web UI, owns the SQLite store, manages scan state, starts reconstruction subprocesses, probes ROS, and can control a picker-side process through ROS or the Docker socket. That coupling is migration evidence, not the new runtime boundary. The accepted design keeps the useful single-operator scan workflow in the ObjectDB companion, reuses normal robot/camera/perception adapters, isolates reconstruction in an on-demand worker, and keeps Compose control outside the app. Runtime task loading is local-file access and has no ObjectDB, Docker-socket, database, lease, Git, or sync dependency. Optional Git/LFS backup remains an authoring concern and does not require a permanent sidecar.
