...
Pin Isaac ROS 4.6 and reuse its supported SAM2 and FoundationPose packages. Do not wrap NVIDIA nodes merely to rename them. Add only configuration, launch composition, message normalization where a shared toolbox contract requires it, and health logic not supplied upstream. CuMotion is outside v1.
...
| Multi-body scene tracking | Not satisfied by the selected FoundationPose graph alone | Project M3T integration behind the shared tracking contract |
...
| NITROS transport | Supported within NVIDIA graphs | Preserve NITROS inside the perception service; cross the toolbox boundary only where standard ROS messages are needed |
...
- Record base-image digest, driver version, CUDA version, TensorRT version, GPU ID, model hashes, engine-build arguments, and package versions in every qualification report.
...
The adapter owns object prompts, prompt-to-object ID mapping, mask label validation, timeout, and memory diagnostics. It does not own the SAM2 model or inference implementation. Inputs are synchronized RGB plus a typed point or box prompt. Output is a mask with object ID, source timestamp, camera frame, confidence evidence when available, and implementation metadata.
...
The adapter resolves an approved object revision to its mesh, scale, canonical frame, symmetry rules, and current segmentation mask. It validates rectified RGB/depth/CameraInfo timestamps before submitting estimation. The normalized result contains the object ID and revision, pose, frame, timestamp, upstream score fields, implementation ID, and raw-result reference.
...
- The pinned perception image builds on the desktop and records a dependency manifest.
