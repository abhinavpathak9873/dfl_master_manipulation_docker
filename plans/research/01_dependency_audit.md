...
Every external dependency is locked by a manifest plus image digest or source commit. An upgrade packet records the old and new pins, upstream changes, build results, contract tests, benchmark deltas, data migrations, and rollback. Floating branch or latest-at-build resolution is prohibited.
