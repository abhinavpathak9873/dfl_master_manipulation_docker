...
- Ubuntu 24.04/Jazzy core Dockerfile and lock manifest
...
- `compose.yaml` services named `core`, `perception`, `genesis`, `objectdb`, `scan-worker`, `objectdb-sync`, and per-host gateways
...
3. Keep Isaac ROS, Genesis, and scan reconstruction in separate images. Mount project source read-only where possible and write engines, logs, caches, build outputs, and scan staging to service-specific persistent paths.
...
5. Mount `data/object_db` read-write only in `objectdb`; mount its approved assets read-only in core, perception, and scan-worker. Mount `data/scan_staging` read-write only in ObjectDB and scan-worker.
...
10. Add deterministic clean-build and incremental-build commands. Never resolve a floating dependency during a release build.
...
- Shared mount contract for `config`, `scenes`, the `object_db` repository at `data/object_db`, `data/scan_staging`, `models`, `calibration`, and `logs/runs`
